...
export class MessageService {
  ...
  async deleteMessage(chatId: number, userId: number, messageId: number) {
    return await AppDataSource.transaction(async (transactionalEntityManager) => {
      const chatMemberRepository = transactionalEntityManager.getRepository(ChatMember);
      const messageRepository = transactionalEntityManager.getRepository(Message);
      const messageStatusRepository = transactionalEntityManager.getRepository(MessageStatus);
      ...
      const newStatuses = chatMembers
        .filter((member) => !messageStatuses.some((status) => status.user_id === member.user_id))
        .map((member) =>
          messageStatusRepository.create({
